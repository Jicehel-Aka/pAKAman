...
void Pacman::update(GameState& g)
{
    ...
    // --------------------------------------------------------
    // Entrees : on REUTILISE l'etat deja lu par app_main ce frame (g_keys), au
    // lieu de rappeler input_poll() -> g_core.pool(). Ainsi le bus I2C+ADC n'est
    // lu qu'UNE fois par frame (principe "proprietaire unique" du composant).
    const Keys& k = g_keys;
